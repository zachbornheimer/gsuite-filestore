...
class Zysys_FileStore {
    ...
    public function configure() {
        ...
        if ($this->client_secret) {
            $client->setAuthConfig(json_decode($this->client_secret, true));
        } else {
    ...
    public function add_row(...$args) {
        ...
        foreach (['gsheet_id', 'gsheet_sheet_name', 'gsheet_row_identifier'] as $var) {
            if (!$this->__get($var)) {
        ...
        $body = new Google_Service_Sheets_ValueRange(['values' => [$args]]);
    ...
    public function create_drive_subfolder($name, $switch_to = false) {
        ...
        $fileMetadata = new Google_Service_Drive_DriveFile([
            'name' => $name,
            'parents' => [$this->__get("drive_parent_id")],
            'mimeType' => 'application/vnd.google-apps.folder'
        ]);

        $file = $this->__get("driveAgent")->files->create($fileMetadata, ['fields' => 'id']);
    ...
    public function store_drive_file($filepath) {
        ...
        $file = $this->__get("driveAgent")->files->create($fileMetadata, [
